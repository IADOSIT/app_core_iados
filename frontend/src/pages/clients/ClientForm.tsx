...
export default function ClientForm({ client, onSuccess, onCancel }: ClientFormProps) {
  ...
  const { register, handleSubmit, watch, formState: { errors } } = useForm({
    defaultValues: {
      ...
      companyName: (client as any)?.company_name || client?.companyName || '',
      rfc: client?.rfc || '',
      industry: client?.industry || '',
      website: client?.website || '',
      firstName: (client as any)?.first_name || client?.firstName || '',
      lastName: (client as any)?.last_name || client?.lastName || '',
      email: client?.email || '',
      phone: client?.phone || '',
      ...
      postalCode: (client as any)?.postal_code || client?.postalCode || '',
      profitPersonCount: (client as any)?.profit_person_count || client?.profitPersonCount || 2,
      notes: client?.notes || '',
      adminUser: (client as any)?.admin_user || (client as any)?.adminUser || '',
      adminPassword: '',
